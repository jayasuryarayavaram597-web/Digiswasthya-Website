...
export function PortalDoctorsSpecialtyChart({ data }: PortalDoctorsSpecialtyChartProps) {
    ...
    return (
        ...
        >
            ...
            {/* Compact Container */}
            <div className="relative pt-2 pb-2 px-3 border-b border-l border-slate-200">
                <div className="space-y-2">
                    {displayData.map((item, idx) => {
                        ...
                        return (
                            ...
                            >
                                {/* Left Label */}
                                <div className="w-44 text-right text-[11px] font-semibold text-slate-600 truncate shrink-0" title={item.specialty}>
                                    {item.specialty}
                                </div>
                                ...
                                <div className="flex-1 h-3.5 bg-slate-100/60 rounded-full relative flex items-center overflow-hidden">
                                    <motion.div
                                        ...
                                        viewport={{ once: true }}
                                        transition={{ duration: 0.8, delay: Math.min(idx * 0.02, 0.4), ease: [0.16, 1, 0.3, 1] }}
                                ...
                                {isHovered && (
                                    <motion.div
                                        ...
                                        className="absolute left-[45%] -top-3 bg-white/95 backdrop-blur-md border border-slate-200 text-slate-900 text-xs font-bold px-3 py-1.5 rounded-xl shadow-lg z-20 pointer-events-none flex items-center gap-2"
                                    >
                                        <span className="w-2 h-2 rounded-full bg-emerald-600 shrink-0" />
                                        <span className="truncate max-w-[200px]">{item.specialty}:</span>
                ...
                {/* X Axis Scale Labels */}
                <div className="flex justify-between text-[9.5px] font-semibold text-slate-400 mt-4 pl-48 pr-2">
                    <span>0</span>
                    <span>6</span>
                    <span>12</span>
                    <span>18</span>
