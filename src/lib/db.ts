...
/* ─────────────────────────────────────────────────────────────
   IMPACT DATA — Field team uses these to update live stats
   ───────────────────────────────────────────────────────────── */

export interface ImpactSummaryUpdate {
    ...
    // Growth trends array (one entry per year)
    growthTrends?: { year: string; patients: number; camps: number; doctors: number }[];
    // Regional reach
    regionalReach?: { district: { en: string; hi: string }; count: number }[];
}

/**
 * updateImpactStats
 * Field team calls this to update live KPI numbers on the website.
 * Automatically updates the "Last updated" timestamp shown to donors.
 */
export async function updateImpactStats(update: ImpactSummaryUpdate) {
    try {
        ...
        console.log("[Firestore] Impact stats updated.");
        return true;
    } catch (error) {
        console.error("[Firestore Error] Failed to update impact stats:", error);
        throw error;
    }
}

export interface CentreRecord {
    id: string;           // e.g. "DS-TMC-021"
    name: string;         // Centre name
    district: { en: string; hi: string };
    state: { id: string; en: string; hi: string };
    ...
    pinLabel: string;
    color: string;        // Tailwind fill class for the state on map
    accentColor: string;
}

/**
 * addCentre
 * Field team calls this when a new telemedicine centre opens.
 * The centre automatically appears on the website map.
 */
export async function addCentre(centre: CentreRecord) {
    try {
        const centreRef = doc(db, "centres", centre.id);
        await setDoc(centreRef, {
            ...centre,
            createdAt: serverTimestamp(),
        });
        console.log(`[Firestore] Centre added: ${centre.id}`);
        return true;
    } catch (error) {
        console.error("[Firestore Error] Failed to add centre:", error);
        throw error;
    }
}
...
/**
 * updateCentreStats
 * Update monthly patient count for a specific centre.
 */
export async function updateCentreStats(centreId: string, patientsThisMonth: number, campsTotal: number) {
    try {
        ...
        console.log(`[Firestore] Centre stats updated: ${centreId}`);
        return true;
    } catch (error) {
        console.error("[Firestore Error] Failed to update centre stats:", error);
        throw error;
    }
}
